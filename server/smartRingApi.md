...
```text
/smartRing/admin/blessings?page=1
```

页面按接收时间倒序列出每次祈福事件，包含记录 ID、发送方、接收方、贴纸昵称、祝福语、贴纸 UUID、APP 包名和自祈福标记，并统计总次数、发送用户数、接收用户数和自祈福次数。每页最多 100 条，通过上一页、下一页查看全部历史记录。该页面必须先登录管理后台。
